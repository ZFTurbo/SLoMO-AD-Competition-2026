...
def proc_with_qwen():
    ...
    valid['video_id'] = valid['youtube_id']
    videos_to_proc = valid['video_id'].unique()
    print(len(videos_to_proc))

    character_map = json.load(open(INPUT_PATH + '/SF20K-AD/sf20kad_private_set_charbank.json', 'r'))

    N_FRAMES = 50
    out_file = './' + os.path.basename(input_file)[:-4] + '_{}_{}_char.csv'.format(N_FRAMES, model_name)
    ...
    for video_id in videos_to_proc:
        ...
        video_path = INPUT_PATH + '/SF20K-AD/sf20kad_private_set_video/' + video_id + '.mp4'
        part = valid[valid['video_id'] == video_id]
        chars = character_map[video_id]
        print(chars)
        print("Characters in movie: {}".format(len(chars)))

        actors_data = []
        for c in chars:
            im_path = os.path.abspath(INPUT_PATH + '/SF20K-AD/' + c['image'])
            if not os.path.exists(im_path):
                print(im_path)
                print("No actor image {}...".format(c['name']))
                continue
            actors_data.append({"image": im_path, "name": c['name']})
            if len(actors_data) > 25:
                break

        print("Actors data: {}".format(len(actors_data)))

        df = pd.read_csv(INPUT_PATH + '/SF20K-AD/audios_vocals/' + video_id_small + '/vocals_mono_text_segment.csv')
        df = df.values
        txt_arr = []
